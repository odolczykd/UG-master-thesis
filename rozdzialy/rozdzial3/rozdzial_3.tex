...
\begin{itemize}
    \item mechanizm \emph{wirtualnych wątków} (ang. \emph{virtual threads}), będący alternatywą dla klasycznego mechanizmu wątków platformowych i zdefiniowany w~dokumencie JEP\footnote{JEP (\emph{JDK Enhancement Proposal}) to formalny dokument opisujący proponowane zmiany, nowe funkcjonalności lub usprawnienia platformy JDK.} 444 \cite{jep444},
...
W celu wyjaśnienia potrzeby wprowadzenia mechanizmu wirtualnych wątków do środowiska JVM, dokument \textit{JEP 444: Virtual Threads} \cite{jep444} powołuje się na prawo Little'a \cite{little2015}, które wiąże ze sobą latencję, współbieżność i przepustowość w kontekście skalowalności aplikacji współbieżnych:
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{rozdzialy/rozdzial3/img/VT-lifecycle.pdf}
...
\begin{enumerate}
    ...
    \item Gdy wątek wirtualny $VT_i$ wykonuje operację blokującą (stan reprezentowany przez symbol \faLock), JVM zapisuje jego aktualny stan, dodaje go na koniec kolejki $Q_{\mathrm{FIFO}}$, jednocześnie zwalniając (ang. \emph{unmount}) wątek nośny $CT_j$
    \item JVM przypisuje kolejny wątek wirtualny (tj. pierwszy w kolejce $Q_{\mathrm{FIFO}}$) do wątku nośnego $CT_j$
\end{enumerate}

\subsubsection{Pule wirtualnych wątków}
bla bla bla
