...
\subsubsection{Tryb pomiaru przepustowości (\textbftt{Throughput})}
Tabele \ref{table:benchmark-A-thrpt-Q1}, \ref{table:benchmark-A-thrpt-Q2} i \ref{table:benchmark-A-thrpt-Q3} oraz odpowiadające im wykresy przedstawione na rysunkach \ref{fig:benchmark-A-thrpt-Q1}, \ref{fig:benchmark-A-thrpt-Q2} i \ref{fig:benchmark-A-thrpt-Q3} prezentują wyniki benchmarku przeprowadzonego w ramach scenariusza $\mathcal{A}$ w~trybie pomiaru przepustowości, tj. w trybie \texttt{Throughput}. Wartości przedstawione w~tabelach i na odpowiadających im wykresach zostały wyznaczone w~zależności od wariantu modelu współbieżności, poziomu współbieżności i zapytania.
...
% \vspace{1em}
% \noindent
% \textit{Dla zachowania czytelności wyniki znajdują się na następnej stronie.}
...
\begin{figure}[p]
    ...
    \begin{tabularx}{\linewidth}{|c|>{\centering\arraybackslash}X|>{\centering\arraybackslash}X|>{\centering\arraybackslash}X|}
        \hline
        \textbf{Poz. współb.} & \textbftt{JDBC+PT} & \textbftt{R2DBC} & \textbftt{JDBC+VT} \\
        \hline
...
\newpage
\subsubsection{Tryb próbkowania czasu (\textbftt{SampleTime})}

Tabele \ref{table:benchmark-A-sample-Q1}, \ref{table:benchmark-A-sample-Q2} i \ref{table:benchmark-A-sample-Q3} oraz odpowiadające im wykresy przedstawione na rysunkach \ref{fig:benchmark-A-sample-Q1}, \ref{fig:benchmark-A-sample-Q2} i \ref{fig:benchmark-A-sample-Q3} prezentują wyniki benchmarku przeprowadzonego w ramach scenariusza $\mathcal{A}$ w~trybie próbkowania czasu, tj. w trybie \texttt{SampleTime}. Wartości przedstawione w~tabelach i na odpowiadających im wykresach zostały wyznaczone w~zależności od wariantu modelu współbieżności, poziomu współbieżności i zapytania.
...
\begin{enumerate}[label=(\roman*)]
    \item Wyniki we wspomnianych wyżej tabelach uwzględniają średni czas (AVG) i~wartości percentyli P50, P95, P99 czasu wykonania badanych wariantów scenariusza i podane są w sekundach. Wykresy przedstawione na odpowiadających im rysunkach prezentują te dane na osi pionowej w jednostce oznaczonej jako [s].
...
\vspace{1em}
\noindent
\textit{Dla zachowania czytelności wyniki znajdują się na następnych stronach.}

% ODKOMENTOWAĆ DO DRUKU
% \clearpage
% \thispagestyle{empty}
% \null
% \clearpage
...
\begin{table}[p]
    ...
    \begin{tabularx}{\linewidth}{|c|c|>{\centering\arraybackslash}X|>{\centering\arraybackslash}X|>{\centering\arraybackslash}X|}
        \hline
        \textbf{Poz. współb.} & \textbf{Metryka} & \textbftt{JDBC+PT} & \textbftt{R2DBC} & \textbftt{JDBC+VT} \\
        \hline
...
\subsubsection{Tryb próbkowania czasu (\textbftt{SampleTime})}
Tabela \ref{table:benchmark-B-sample} oraz odpowiadające jej wykresy przedstawione na rysunku \ref{fig:benchmark-B-sample} prezentują wyniki benchmarku przeprowadzonego w ramach scenariusza $\mathcal{B}$ w~trybie próbkowania czasu, tj. w trybie \texttt{SampleTime}. Wartości przedstawione w~tabeli i~na odpowiadających jej wykresach zostały wyznaczone w~zależności od wariantu modelu współbieżności i konfiguracji serwisów.

\noindent
Dodatkowe uwagi do tabeli \ref{table:benchmark-B-sample}:
\begin{enumerate}[label=(\roman*)]
    \item Wyniki we wspomnianej wyżej tabeli uwzględniają średni czas (AVG) i~wartości percentyli P50, P95, P99, P99.9 czasu wykonania badanych wariantów scenariusza i podane są w sekundach. Wykresy przedstawione na odpowiadającym jej rysunku prezentują te dane na osi pionowej w jednostce oznaczonej jako [s].
    \item Wyniki we wierszach oznaczonych przez AVG we wspomnianej wyżej tabeli są przedstawione w formacie $S \pm E$, gdzie $S$ oznacza uzyskany wynik, a~$E$ oznacza błąd pomiaru.
    \item Wyniki i wartości błędów we wspomnianej wyżej tabeli zapisane zostały z~kropką jako separatorem dziesiętnym, z dokładnością do części tysięcznych, tj. $0.001$.
\end{enumerate}
...
\begin{table}[p]
    ...
    \begin{tabularx}{\linewidth}{|c|c|>{\centering\arraybackslash}X|>{\centering\arraybackslash}X|>{\centering\arraybackslash}X|}
        \hline
        \textbf{Konfig. serwisów} & \textbf{Metryka} & \textbftt{Legacy} & \textbftt{Reactive} & \textbftt{Loom} \\
        \hline
        ...
        \hline
    \end{tabularx}
    \caption{Wartości średniego czasu i percentyli P50, P95, P99, P99.9 dla scenariusza $\mathcal{B}$ w~zależności od konfiguracji serwisów i~modelu współbieżności}
    \source{opracowanie własne}
...
\begin{figure}[p]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        ...
        \caption{konfiguracja \texttt{FIVE\_SIMILAR}}
        \label{fig:benchmark-B-sample-C3}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{rozdzialy/rozdzial6/wykresy/B_plot_avg_C4.pdf}
        \caption{konfiguracja \texttt{OUTLIER\_HEAVY}}
        \label{fig:benchmark-B-sample-C4}
    \end{subfigure}
    \caption{Wykresy kolumnowe średniego czasu i percentyli P50, P95, P99, P99.9 dla scenariusza $\mathcal{B}$ w~zależności od konfiguracji serwisów i~modelu współbieżności}
    \source{opracowanie własne}
...
\subsubsection{Zapytanie \textbftt{Q1} (proste pobranie danych po kluczu głównym)}

Warianty \texttt{JDBC+VT} i~\texttt{R2DBC} osiągnęły zbliżone wartości przepustowości na~każdym poziomie współbieżności, wyraźnie wyprzedzając wariant \texttt{JDBC+PT}: przy poziomie współbieżności równym 16 o~około 76\%, zaś przy poziomie równym 1024 o~około 151\%. Różnice między wariantami \texttt{JDBC+VT} a~\texttt{R2DBC} mieszczą się w~granicach błędów pomiarowych. Wyniki uzyskane w ramach trybu próbkowania czasu (\texttt{SampleTime}) potwierdzają tę~obserwację, tj. przy poziomie współbieżności równym 1024 warianty \texttt{JDBC+VT} i~\texttt{R2DBC} uzyskały wartości średniego czasu o około 63\% mniejsze niż wariant \texttt{JDBC+PT}. Ponadto wartości percentyli P50, P95 i P99 dla wariantów \texttt{JDBC+VT} i~\texttt{R2DBC} są zbliżone i~wyraźnie mniejsze w~porównaniu do wariantu \texttt{JDBC+PT}.

\subsubsection{Zapytanie \textbftt{Q2} (prosta zmiana wartości po kluczu głównym)}

Wyniki dla zapytania \texttt{Q2} wykazują podobny trend do wyników zapytania \texttt{Q1}, lecz przy wyższej zmienności, tj. przy niskich poziomach współbieżności błędy pomiaru osiągają wysoką wartość. Przy wyższych poziomach wyniki stabilizują się, tj. wariant \texttt{JDBC+VT} osiągnął najwyższą przewagę przy poziomach współbieżności równych 32 i~1024. Wyniki uzyskane w~ramach trybu próbkowania czasu pokrywają się z~tą obserwacją. We~wszystkich metrykach wariant \texttt{JDBC+VT} uzyskał wyniki nieznacznie lepsze od~wariantu \texttt{R2DBC}, a~oba przewyższają wariant \texttt{JDBC+PT} na~każdym badanym poziomie współbieżności.

\subsubsection{Zapytanie \textbftt{Q3} (złożone zapytanie z~agregacją i~wielokrotnym złączeniem)}

Przy poziomach współbieżności od~16 do~256 wyniki wszystkich trzech wariantów są zbliżone, mieszcząc się w~granicach błędów pomiaru. Przy poziomie równym 1024 pojawia się wyraźna anomalia: przepustowość wariantu \texttt{R2DBC} jest ponad dwukrotnie niższa w~porównaniu do pozostałych wariantów, a~w~trybie próbkowania czasu wyniki wskazują na ponad dwukrotny wzrost wartości średniego czasu i percentyli względem wariantów \texttt{JDBC+PT} i \texttt{JDBC+VT}. Jest to~jedyna konfiguracja w~scenariuszu $\mathcal{A}$, w~której wariant \texttt{R2DBC} uzyskał wyniki istotnie gorsze od~wariantów opartych o~sterownik JDBC.

\newpage
\subsection{Scenariusz \ensuremath{\mathcal{B}}}

We wszystkich czterech konfiguracjach serwisów warianty \texttt{Reactive} i~\texttt{Loom} uzyskały niemal identyczne wyniki, a różnice między nimi mieszczą się w~granicach błędu pomiarowego. Wariant \texttt{Legacy} był wolniejszy od pozostałych wariantów we~wszystkich konfiguracjach, przy czym skala dysproporcji różni się w~zależności od~konfiguracji serwisów:
\begin{itemize}
    \itemsep0em
    \item w konfiguracji \texttt{THREE\_FAST\_TWO\_SLOW} średnio trzykrotnie wolniejszy,
    \item w konfiguracji \texttt{ONE\_FAST\_FOUR\_SLOW} średnio około 28\% wolniejszy (najmniejsza różnica),
    \item w konfiguracji \texttt{FIVE\_SIMILAR} średnio około 36\% wolniejszy,
    \item w konfiguracji \texttt{OUTLIER\_HEAVY} średnio ponad trzykrotnie wolniejszy (największa różnica),
\end{itemize}

We~wszystkich czterech konfiguracjach percentyle P95, P99 i P99.9 dla wariantu \texttt{Legacy} są istotnie wyższe niż dla wariantów \texttt{Reactive} i~\texttt{Loom}, a~ich wartości skupiają się blisko wartości $L_{\text{outlier}}$ dla najwolniejszego serwisu w~danej konfiguracji. Ponadto konfiguracja \texttt{OUTLIER\_HEAVY} wyróżnia się istotną rozbieżnością między wartością percentyla P50 a~wartością AVG dla~wariantu \texttt{Legacy}.