...
\newpage
Z powyższego opisu wynika, że metryką mierzoną przez benchmark uruchomiony w trybie \texttt{Throughput} jest \emph{przepustowość} (oznaczana przez THRPT), czyli liczba operacji wykonanych przez badany fragment kodu w jednostce czasu. Tryb ten został użyty przy pomiarze kodu zaimplementowanego w ramach scenariusza $\mathcal{A}$.
...
Wynikiem jest średni czas wykonania operacji wraz z oszacowaniem błędu pomiarowego. Dodatkowo JMH raportuje rozkład zebranych próbek, w tym wartości minimalne i maksymalne oraz percentyle.
Jak podaje Luszniewicz \cite{luszniewicz1970}, mianem \emph{percentyla rzędu $p$}, gdzie $p \in [0, 100]$, określa się taką wartość $x$, że co najmniej $p\%$ obserwacji ma wartość mniejszą lub równą $x$, a co najwyżej $(100-p)\%$ obserwacji ma wartość większą lub równą $x$.

Na podstawie powyższych informacji definiuje się dwie metryki:
\begin{itemize}
    \itemsep0em
    \item P$n$, która określa percentyl rzędu $n$ czasu wykonania badanego fragmentu kodu,
    \item AVG, która określa średni czas wykonania badanego fragmentu kodu.
\end{itemize}
 Wartości te pozwalają ocenić latencję i zachowanie w~ogonie rozkładu \cite{jmh-documentation}. Tryb \texttt{SampleTime} został użyty przy pomiarze kodu zaimplementowanego w ramach obu scenariuszy.
...
\begin{itemize}
    ...
    \item \mintinline{java}|Threads| -- liczba wątków jednocześnie wykonujących kod: 1.
\end{itemize}
Podobnie jak w ramach scenariusza $\mathcal{A}$, liczba wątków w powyższej konfiguracji została ustawiona na 1 ze względu na fakt, że zarządzanie liczbą i rodzajem wątków zostało przeniesione bezpośrednio do metod, na których wykonywane są benchmarki. Parametr ten reprezentowany jest przez liczbę równolegle uruchamianych serwisów w ramach jednego wariantu. W przypadku scenariusza $\mathcal{B}$ wartość ta wynosi 5. Implementacja tej logiki różni się w zależności od wariantu modelu współbieżności, co zostało szczegółowo przedstawione w rozdziale piątym, w~sekcji \hyperref[sec:description-scenario-B]{5.2.2 Scenariusz \ensuremath{\mathcal{B}}: Agregacja wyników oparta na mechanizmie kworum}.

Z racji, że warianty scenariusza $\mathcal{B}$ uruchomiono w trybie \texttt{SampleTime}, mierzonymi w~jego ramach metrykami są AVG, P50, P95, P99 i P99.9.
...
% TODO: wrzucić dane AI, zobaczyć jak zinterpretuje wyniki (na co zwróci uwagę) i opisać to sensownie
% TODO: czyste porównanie wartości, bez rekomendacji i szerszych wniosków
...
% dla kazdej hipotezy: potwierdzona / odrzucona / częściowo powtierdzona + krótkie uzasadnienie