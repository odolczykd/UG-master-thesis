...
Użyto również dwóch narzędzi do statycznej analizy kodu: SonarQube i PMD, [opis narzędzi jak wyżej].

\subsection{Opis użytych trybów benchmarków i definicja mierzonych przez nie metryk}
Do uruchomienia benchmarków przeprowadzonych na podstawie scenariuszy $\mathcal{A}$ i $\mathcal{B}$ wykorzystano dwa tryby: \texttt{Throughput} i \texttt{SampleTime}.
...
Z powyższego opisu wynika, że metryką mierzoną przez benchmark uruchomiony w trybie \texttt{Throughput} jest \emph{przepustowość}, czyli liczba operacji wykonanych przez badany fragment kodu w jednostce czasu, oznaczana jako THRPT. Tryb ten został użyty przy pomiarze kodu zaimplementowanego w ramach scenariusza $\mathcal{A}$.
...
\subsection{Definicja metryk statycznej analizy kodu}
% TODO
% narzędzia: SonarQube, PMD - zaimplementować je, puścić analizę, sprawdzić co mierzą
% spisać metryki
...
\begin{itemize}
    ...
    \item w ramach statycznej analizy kodu: ?.
\end{itemize}

\subsection{Szczegóły pomiaru dla scenariusza \ensuremath{\mathcal{B}}}

Metrykami mierzonymi w ramach scenariusza $\mathcal{B}$ są:
\begin{itemize}
    \itemsep0em
    \item dla trybu \texttt{SampleTime}: AVG, P50, P95, P99 i P99.9,
    \item w ramach statycznej analizy kodu: ?.
\end{itemize}
...
\newpage
\subsubsection{Statyczna analiza kodu}
