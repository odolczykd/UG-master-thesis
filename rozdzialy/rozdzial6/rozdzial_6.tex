...
Szósty rozdział pracy prezentuje wyniki benchmarków przeprowadzonych w~ramach scenariuszy $\mathcal{A}$ i~$\mathcal{B}$. Obejmuje on szczegółową prezentację wyników pomiarów w postaci tabel i wykresów, a także ich analizę porównawczą.
...
\begin{table}[p]
    ...
    \begin{tabularx}{\linewidth}{|c|c|>{\centering\arraybackslash}X|>{\centering\arraybackslash}X|>{\centering\arraybackslash}X|}
        \hline
        \textbf{Konfig. serwisów} & \textbf{Metryka} & \textbf{Legacy} & \textbf{Reactive} & \textbf{Loom} \\
        \hline
...
\subsubsection{Zapytanie \texttt{Q1} (proste pobranie danych po kluczu głównym)}
...
\subsubsection{Zapytanie \texttt{Q2} (prosta zmiana wartości po kluczu głównym)}
...
\subsubsection{Zapytanie \texttt{Q3} (złożone zapytanie z~agregacją i~wielokrotnym złączeniem)}
...
We wszystkich czterech konfiguracjach serwisów warianty \texttt{Reactive} i~\texttt{Loom} uzyskały niemal identyczne wyniki, a różnice między nimi mieszczą się w~marginesie błędu pomiarowego. Wariant \texttt{Legacy} był wolniejszy od pozostałych wariantów we~wszystkich konfiguracjach, przy czym skala dysproporcji różni się w~zależności od~konfiguracji serwisów:
\begin{itemize}
    \itemsep0em
