...
\begin{itemize}
    \itemsep0em
    \item P$n$, która określa się percentyl rzędu $n$ czasu wykonania badanego fragmentu kodu,
    \item AVG, która określa się średni czas wykonania badanego fragmentu kodu.
\end{itemize}
...
\begin{itemize}
    ...
    \item \mintinline{java}|Threads| -- liczba wątków jednocześnie wykonujących kod: 1.
\end{itemize}
Podobnie jak w ramach scenariusza $\mathcal{A}$, liczba wątków w powyższej konfiguracji została ustawiona na 1 ze względu na fakt, że zarządzanie liczbą i rodzajem wątków zostało przeniesione bezpośrednio do metod, na których wykonywane są benchmarki. Parametr ten reprezentowany jest przez liczbę równolegle uruchamianych serwisów w ramach jednego wariantu (tu: 5). Implementacja tej logiki różni się w zależności od wariantu modelu współbieżności, co zostało szczegółowo przedstawione w~sekcji \hyperref[sec:description-scenario-B]{5.2.1 Scenariusz \ensuremath{\mathcal{B}}: Agregacja wyników oparta na mechanizmie kworum}.

Z racji, że warianty scenariusza $\mathcal{B}$ uruchomiono jedynie w trybie \texttt{SampleTime}, mierzonymi w jego ramach metrykami są AVG, P50, P95, P99 i P99.9.
