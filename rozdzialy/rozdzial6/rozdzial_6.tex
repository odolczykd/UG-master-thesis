...
Szósty rozdział pracy prezentuje wyniki benchmarków przeprowadzonych w~ramach scenariuszy $\mathcal{A}$ i~$\mathcal{B}$. Metodyka pomiarów, konfiguracja narzędzia JMH oraz przyjęte kryteria oceny zostały omówione w rozdziale piątym. Rozdział obejmuje prezentację wyników pomiarów, ich analizę porównawczą oraz weryfikację hipotez badawczych.
...
Warianty \texttt{JDBC+VT} i~\texttt{R2DBC} osiągnęły zbliżone wartości przepustowości na~każdym poziomie współbieżności, wyraźnie wyprzedzając wariant \texttt{JDBC+PT}: przy poziomie współbieżności równym 16 o~około 76\%, zaś przy poziomie równym 1024 o~około 151\%. Różnice między wariantami \texttt{JDBC+VT} a~\texttt{R2DBC} mieszczą się w~granicach błędów pomiarowych. Wyniki uzyskane w ramach trybu próbkowania czasu (\texttt{SampleTime}) potwierdzają tę~obserwację, tj. przy poziomie współbieżności równym 1024 wariant \texttt{JDBC+PT} uzyskał wartość średniego czasu równą 0.159~s, natomiast warianty \texttt{JDBC+VT} i~\texttt{R2DBC} wartości o około 63\% mniejsze -- odpowiednio 0.060~s i~0.058~s. Ponadto wartości percentyli P50, P95 i P99 dla wariantów \texttt{JDBC+VT} i~\texttt{R2DBC} są zbliżone i~wyraźnie mniejsze w~porównaniu do wariantu \texttt{JDBC+PT}.

\subsubsection{Zapytanie \texttt{Q2} (prosta zmiana wartości po kluczu głównym)}

Wyniki dla zapytania \texttt{Q2} wykazują podobny trend do wyników zapytania \texttt{Q1}, lecz przy wyższej zmienności, tj. przy niskich poziomach współbieżności błędy pomiaru osiągają wysoką wartość. Przy wyższych poziomach wyniki stabilizują się, tj. wariant \texttt{JDBC+VT} osiągnął najwyższą przewagę przy poziomach współbieżności równych 32 (157.456~ops/s wobec 122.683~ops/s dla wariantu \texttt{R2DBC} i~107.766~ops/s dla wariantu \texttt{JDBC+PT}) i~1024 (5.042~ops/s wobec 4.600~ops/s i~3.736~ops/s, odpowiednio dla wariantów \texttt{R2DBC} i \texttt{JDBC+PT}). Wyniki uzyskane w~ramach trybu próbkowania czasu (\texttt{SampleTime}) potwierdzają tę~obserwację. We~wszystkich metrykach wariant \texttt{JDBC+VT} uzyskał wyniki nieznacznie lepsze od~\texttt{R2DBC}, a~oba przewyższają wariant \texttt{JDBC+PT} na~każdym badanym poziomie współbieżności.

\subsubsection{Zapytanie \texttt{Q3} (złożone zapytanie z~agregacją i~wielokrotnym złączeniem)}

Przy poziomach współbieżności od~16 do~256 wyniki wszystkich trzech wariantów mieszczą się w~granicach błędów pomiaru. Przy poziomie równym 1024 pojawia się wyraźna anomalia: przepustowość wariantu \texttt{R2DBC} jest ponad dwukrotnie niższa w~porównaniu do pozostałych wariantów, a~w przypadku trybu próbkowania czasu wyniki wskazują na ponad dwukrotny wzrost wartości średniego czasu i percentyli względem wariantów \texttt{JDBC+PT} i \texttt{JDBC+VT}. Jest to~jedyna konfiguracja w~scenariuszu $\mathcal{A}$, w~której wariant \texttt{R2DBC} uzyskał wyniki istotnie gorsze od~wariantów opartych o~sterownik JDBC.

\subsection{Scenariusz \ensuremath{\mathcal{B}}}

We wszystkich czterech konfiguracjach warianty \texttt{Reactive} i~\texttt{Loom} uzyskały niemal identyczne wyniki, a różnice między nimi mieszczą się w~marginesie błędu pomiarowego. Wariant \texttt{Legacy} był wolniejszy we~wszystkich konfiguracjach, przy czym skala dysproporcji różni się w~zależności od~konfiguracji serwisów.

Największe różnice w~wartości średniego czasu (AVG) zaobserwowano w~konfiguracjach \texttt{THREE\_FAST\_TWO\_SLOW} i~\texttt{OUTLIER\_HEAVY}. Różnice w wartości metryki AVG dla~konfiguracji \texttt{ONE\_FAST\_FOUR\_SLOW} i~\texttt{FIVE\_SIMILAR} są mniejsze, lecz wyraźniej zaznaczają się w~ogonie rozkładu. We~wszystkich czterech konfiguracjach percentyle P95, P99 i P99.9 dla wariantu \texttt{Legacy} są istotnie wyższe niż dla wariantów \texttt{Reactive} i~\texttt{Loom}, a~ich wartości skupiają się blisko wartości $L_{\text{outlier}}$ dla najwolniejszego serwisu w~danej konfiguracji. Konfiguracja \texttt{OUTLIER\_HEAVY} wyróżnia się ponadto istotną rozbieżnością między wartością percentyla P50 (0.058~s) a~wartością AVG (0.166~s) dla~wariantu \texttt{Legacy}. Przy percentylach P95 i~P99 wszystkie trzy warianty osiągają zbliżone wartości w~przedziale od 0.300~s do 0.302~s, odpowiadającym wartości $L_{\text{outlier}}$ równej $300$~ms dla~serwisu typu \texttt{OUTLIER\_PRONE}.